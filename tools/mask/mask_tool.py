...
_SURNAME_ALT = "|".join(sorted(dict.fromkeys(_COMMON_SURNAMES), key=len, reverse=True))

# スキャン用パターン（会社名・住所は新規、その他は既存パターンから値を拾う）
# 末尾に (?P<v>...) のような明示グループがある場合はその部分のみ、なければ全体一致を候補値とする
SCAN_PATTERNS = [
    ("会社名", r"(?:株式会社|有限会社|合同会社|合資会社|合名会社"
               r"|一般社団法人|公益社団法人|一般財団法人|公益財団法人|特定非営利活動法人|独立行政法人)"
               r"[\w・\-－～（）()]{1,30}"),
    ...
    ("担当者名(ラベル)",   r"(?:担当者?名?|作成者|お客様担当|ご担当)[：:]\s*([^\n\r、。]{2,20})"),

    # 敬称一致（会社名・個人名どちらにも付く。誤検出はやや多いが見逃しを減らす）
    ("会社名/個人名(御中)", r"([^\s、。\n：:]{2,30})\s*御中"),
    ("会社名/個人名(様)",   r"([^\s、。\n：:]{2,20})\s*様(?!邸)"),
