#!/usr/bin/env python3
"""
カタログPDFから表を抽出し、JSON/CSVに変換する（案件ごとの単発利用）。

**用途の限定**
CLAUDE.mdの方針どおり、部品参照の基本は `index.csv` で該当ページを特定し
該当ページのPDFのみを読む方式とする。本スクリプトは「表が複雑でページを
読むだけでは判断しづらい」場合に、その表だけを変換して見るための道具。
カタログ全体を事前に変換して溜め込む使い方はしない。

**出力を信用する前に精度を見ること**
過去に4カタログ350表を一括変換した結果、中央値は93〜99.8%だったが
80%未満が49表、最悪は-318.2%で、セルが断片化して実質使えない表が
混ざっていた。個別CSVには精度が入らないため、CSVだけを見ると
信頼度が判断できない。必ず tables.json の accuracy を確認し、
低い表は元のPDFページを読むこと。
