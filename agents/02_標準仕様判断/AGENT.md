...
- `database/knowledge/customers.csv`（客先ごとの商流・メカ設計の所在・回答リードタイム）
...
- `database/knowledge/customers.csv`（客先ごとの商流・メカ設計の所在）
...
5. **`customers.csv` に同じ客先の実績があれば、商流・メカ設計の所在・回答リードタイムを
   初期値として提供する**（`CLAUDE.md` 設計順序の原則）。
   **これは推定であって確定ではない**。同じ客先でも案件によって商流が変わることがあるため、
   「前回はこうだった」として渡し、Agent01が今回の実態を確認する
...
**メカ設計が誰かは案件ごとに違い、汎用には決められない。** ただし客先ごとには
決まっていることが多いので、実績を残して次回の初期値にする。

| 列 | 内容 | 例 |
|---|---|---|
| `client` | 客先名（`projects_index.csv` の client と一致させる） | abc株式会社 |
| `order_source` | 当社の窓口＝発注元 | 装置メーカー / エンドユーザー / 商社 |
| `mech_owner` | メカ設計の所在（**窓口とは限らない**） | 客先社内 / ○○製作所 / 不明 |
| `mech_contact_route` | メカへの問い合わせ経路と段数 | 直接 / 客先経由(1段) / 商社→客先経由(2段) |
| `answer_lead_days` | 質疑の回答が返るまでの実績日数（平均） | 7 |
| `notes` | 補足（担当者の傾向・答えられない質問の種類 等） | |
| `updated` | 最終更新日 | 2026-07-30 |

### 更新のタイミング

- 案件完了時（蓄積モード）に、実際の商流・回答リードタイムを追記・更新する
- 案件途中でも**商流が判明した時点でAgent01から更新依頼を受けたら反映**する
  （次の案件を待つ必要はない）
- **同じ客先でも案件ごとに商流が変わることがある**。値を上書きするのではなく、
  変わった場合は `notes` に経緯を残す
...
```
project_id,完了日,用途,設備概要,電源電圧,主要モータ台数,主要モータ最大容量[kW],インバータ使用,サーボ使用,安全カテゴリ,CE対応,電線合計_見積[m],電線合計_発注[m],電線合計_実使用[m],製作FB有無,備考
```

| 列名 | 内容 | 例 |
|---|---|---|
| project_id | 案件ID | 20260614-001 |
| 完了日 | 案件完了日(YYYYMMDD) | 20260614 |
| 用途 | 設備種別キーワード(類似検索に使用) | コンベア / 加工機 / 搬送 / 組立 / 検査 |
| 設備概要 | 自由記述の設備説明 | ラインNo.3コンベア(5.5kW×3台) |
| 電源電圧 | AC200V / AC400V 等 | AC200V |
| 主要モータ台数 | 数値 | 3 |
| 主要モータ最大容量[kW] | 1台あたりの最大kW | 5.5 |
| インバータ使用 | yes / no | yes |
| サーボ使用 | yes / no | no |
| 安全カテゴリ | Cat.3 PL d / Cat.1 / なし 等 | なし |
| CE対応 | yes / no | no |
| 電線合計_見積[m] | BOM/見積エージェント出力値 | 150 |
| 電線合計_発注[m] | 実際の発注量合計 | 130 |
| 電線合計_実使用[m] | 製作後の実使用量合計 | 118 |
| 製作FB有無 | manufacturing_feedback.csv に記録済みか | yes / no |
| 備考 | 特記事項 | — |

- 案件完了のたびにAgent02が1行追記する（ユーザー確認後）
- このファイルが提供モードでの検索対象になる
- `用途` と `主要モータ最大容量[kW]` が類似検索の主軸になる
...
- **このシステム（database/projects/ 配下）経由で完了した案件のみを蓄積対象とする**
- Google Drive「参考資料」フォルダへのアクセスは禁止（CLAUDE.mdのアクセス制限ルール参照）
- 過去の案件記録が「参考資料」フォルダに存在していても、Agent02はそこを参照しない
- 初期状態（past_cases_index.csv がヘッダーのみ）では提供モードをスキップしてAgent00に報告する

### 手動シード手順（任意）

ユーザーが過去案件の要約を手動で登録したい場合は、以下の手順で行う。
Agent02は指示を受けたときのみ手動シードを支援する（自律的に実施しない）。

1. ユーザーが案件の概要情報（設備概要・モータ台数・電圧等）を口頭または文書で提供する
2. Agent02が `past_cases_index.csv` への追記案を作成し、ユーザーに確認する
3. ユーザー承認後に追記する
4. Google Driveや社内システムへのアクセスは行わない（ユーザーが情報を手元で用意する）

> **注意**: 手動シードは「参考資料」フォルダの内容をAIが読んで転記することを意味しない。
> ユーザーが自分の手元で確認した情報をAIに伝える形でのみ実施する。
