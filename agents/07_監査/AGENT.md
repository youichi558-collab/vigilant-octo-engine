...
| `audit_start` | 監査を開始したとき（Agent07自身が記録） |
| `audit_complete` | 監査を完了したとき（Agent07自身が記録） |

> 監査エージェント自身もログを残す。監査した事実が記録に残らないと、
> 監査が実施されたかどうかを後から確認できない。
...
| # | 対象 | 内容 | 推奨対応 | 対応状況 |
|---|---|---|---|---|
| V-1 | | | | 未対応 / ユーザー承認待ち / 対応済み |

### Warning（確認推奨）
| # | 対象 | 内容 | 推奨対応 | 対応状況 |
|---|---|---|---|---|
| W-1 | | | | 未対応 / ユーザー承認待ち / 対応済み |
...
**Agent07のファイル操作権限は以下の通り。これは最優先ルールである。**

| 操作対象 | 権限 | 条件 |
|---|---|---|
| 案件データファイル（spec_summary.md / parts_selection.md 等） | **変更禁止** | 内容の修正は責任エージェントが行う |
| `audit_report.md` | 新規作成・更新可 | 監査結果の記録用 |
| `agent_log.csv` | 追記のみ可 | 修正・削除は禁止 |
| 上記以外の既存ファイル | **ユーザー確認後に限り変更可** | 勝手には変更しない |
| ファイルの削除 | **全面禁止** | いかなる状況でも削除しない |

---

## 修正フロー（Violation / Warning 検出時）

```
検出 → audit_report.md に記録 → ユーザーに内容と推奨対応を提示
    → ユーザーが「修正してよい」と指示した場合のみ修正を実施
    → 修正後、audit_report.md の対応欄を更新する
```
